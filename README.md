...
2. Install libraries: `OSCMessage` (CNMAT), `WiFiManager` (tzapu), `NimBLE-Arduino`, `RadioLib`, `Adafruit SSD1306`, `Adafruit GFX`
...
- **Web setup:** menu → *Web Setup* opens the control panel in your browser (the OLED shows the address; in BLE/LoRa modes the device raises its own WiFi network and pauses BLE while setup is open — they share one 2.4 GHz radio). Everything is there: status, mode, region/frequency, BLE keys, a live band spectrum and firmware updates — **including one-click online updates from this repo** when the venue WiFi has internet. In OSC modes the panel is always available at `http://gogo.local` or the board's IP. The **QLab tab** turns your phone into a wireless remote: GO, Panic, transport, the cue list and running cues — the board bridges HTTP to QLab's OSC API. (Joining a network at all — the *WiFi Setup* flow — is the separate WiFiManager-based portal described above.)
...
| LoRa pair (remote + gateway) | Bluetooth keyboard mode |
|---|---|
| ![LoRa scenario](img/scenario-lora.svg) | ![BLE scenario](img/scenario-ble.svg) |

WiFi setup is now the actual WiFiManager captive portal (restyled to match
the brand) - a real screenshot/recording is more honest here than a mockup,
see [docs/osc_setup.md](docs/osc_setup.md) for the step-by-step.

*All screens in every animation are pixel-exact renders of the real firmware
(generated by [docs/generate_demo.py](docs/generate_demo.py)).*
