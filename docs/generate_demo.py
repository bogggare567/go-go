...
/* Волны «летят» от антенны платы к Маку: сдвиг вправо + затухание.
   Три дуги с задержками читаются как движение сигнала, а не пульс на месте. */
@keyframes k-wave {{ 0%,33% {{opacity:0; transform:translateX(0)}} 34.5% {{opacity:.9}} 42% {{opacity:0; transform:translateX(110px)}} 43%,61% {{opacity:0; transform:translateX(0)}} 62.5% {{opacity:.9}} 70% {{opacity:0; transform:translateX(110px)}} 71%,100% {{opacity:0; transform:translateX(0)}} }}
.wave {{ animation: k-wave {DUR} ease-out infinite; transform-box: view-box; }}
.wave2 {{ animation-delay: .22s; }}
.wave3 {{ animation-delay: .44s; }}
...
<!-- radio waves: летят от антенны платы (справа) в сторону Мака -->
<g stroke="#6fd18c" stroke-width="5" fill="none">
  <path class="wave" d="M560 445 a45 45 0 0 1 0 70"/>
  <path class="wave wave2" d="M560 435 a62 62 0 0 1 0 90"/>
  <path class="wave wave3" d="M560 425 a80 80 0 0 1 0 110"/>
</g>
